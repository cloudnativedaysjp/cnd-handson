...
## 実践：デモアプリケーションでCI/CDを体感してみよう
実際に、デモアプリケーションを通じて、CI/CDの導入を体感してみましょう。
ここでは、タスク管理のプロダクトをデモアプリケーションとします。

はじめに、下記のリポジトリをforkしてください。
* アプリケーションのソースコードリポジトリ
* インフラリソースのkubernetesマニフェストリポジトリ
```bash
...
### デモアプリケーションの紹介
本デモアプリケーションは、中規模の複数チームを想定したマイクロサービスとして構成しています。開発成果物は、必ずCI/CDパイプラインを通過することで、複数あるサービスでもテスト・ビルド・デプロイの効率化を実現します。

* 開発言語数：?
* サービス数：?

> [!NOTE]
> アプリケーション詳細仕様は、対象のリポジトリのREADMEに記載しています
