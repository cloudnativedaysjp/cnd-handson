...
> [!NOTE]
>
> 以降のセクションでは、`http://app.envoy-gateway.example.com:8888` でGatewayにアクセスすることを前提としています。
...
```sh
curl -s http://app.envoy-gateway.example.com:8888/
```
...
```sh
curl -s http://app.envoy-gateway.example.com:8888/v1
```
...
```sh
curl -s http://app.envoy-gateway.example.com:8888/v2
```
...
```sh
curl -s --header "x-version: canary" http://app.envoy-gateway.example.com:8888/
```
...
```sh
for i in $(seq 1 10); do curl -s http://app.envoy-gateway.example.com:8888/; done
```
...
```sh
curl -s http://app.envoy-gateway.example.com:8888/headers | head -20
```
