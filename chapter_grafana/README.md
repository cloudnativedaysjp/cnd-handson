...
## 実践: Grafana v12のGit Syncを試す

Grafana v12.0から、DashboardをGitHubリポジトリと直接同期する **Git Sync** 機能がProvisioning経由で提供されるようになりました。
これにより、DashboardをコードとしてGitで管理し、Pull Requestワークフローで変更を追跡することが可能になります。
...
> [!NOTE]
> Git SyncはGrafana OSS/Enterprise版のみ対応で、Grafana Cloudでは利用できません。
> また、執筆時点ではPublic Preview扱いのため、feature flagを有効化する必要があります。
> Grafana UIの表記やfeature flag名はバージョンアップで変わる可能性があるため、うまく動作しない場合は上記リンクの最新情報もあわせて参照してください。

### 前提

- chapter_prometheusまで完了しており、Grafana v12.0以上 (本ハンズオンの時点では v12.2.0) が動作していること
  - 現在のバージョンは以下で確認できます

```shell
kubectl -n prometheus get deployment kube-prometheus-stack-grafana -o jsonpath='{.spec.template.spec.containers[?(@.name=="grafana")].image}'
```

### Step 1: ハンズオンリポジトリのfork

Grafanaから書き込み先となるGitHubリポジトリが必要になるため、本ハンズオンリポジトリをforkします。

1. <https://github.com/cloudnativedaysjp/cnd-handson> にアクセスし、 `Fork` をクリック
2. `Create fork` をクリック
3. 自身のアカウントでforkされていることを確認

> [!TIP]
> chapter_argocd以降でも同じforkを利用します。既にforkしている場合はそのリポジトリを再利用して構いません。

### Step 2: GitHub Personal Access Tokenの発行

Grafanaからforkしたリポジトリに書き込むためのTokenを発行します。

1. <https://github.com/settings/personal-access-tokens/new> にアクセス
2. 以下を設定して `Generate token` をクリック
   - `Token name` ... `grafana-git-sync`
   - `Expiration` ... 任意 (例: 30日)
   - `Repository access` ... `Only select repositories` → forkしたcnd-handsonリポジトリを選択
   - `Repository permissions`
     - `Contents` ... `Read and write`
     - `Metadata` ... `Read-only` (自動で付与されます)
3. 発行されたTokenを控えておきます (この後の画面でしか表示されません)

### Step 3: Grafanaのfeature flagを有効化

kube-prometheus-stackから導入しているGrafanaに対してfeature flagを渡します。
`chapter_prometheus/helm/prometheus-values.yaml` の `grafana:` ブロック配下に以下を追記してください。

```yaml
grafana:
  # ... (既存の adminUser, dashboardProviders, dashboards などはそのまま)
  grafana.ini:
    feature_toggles:
      provisioning: true
...
```shell
cd ../chapter_prometheus/helm
helmfile apply
```

Grafana Podが再起動したことを確認します。

```shell
kubectl -n prometheus rollout status deployment/kube-prometheus-stack-grafana
```

その後、 <http://grafana.example.com/admin/provisioning> にアクセスしてProvisioning画面が表示されればfeature flagが有効になっています。
...
3. 以下を入力
   ...
   - `Branch` ... `main`
   - `Target path` ... `chapter_grafana/dashboards` (任意。空でも可)
4. 同期対象は一旦「全ダッシュボード」を選択
5. `Begin synchronization` をクリック

> [!WARNING]
> 同期開始直後はGrafana上でダッシュボードが一時的に利用不可になる場合があります。完了まで数十秒待ってください。

同期が完了し、Provisioning画面のステータスが `Up-to-date` になれば接続成功です。

### Step 5: DashboardをGit経由で管理してみる

実際にDashboardがGitで管理される様子を確認します。

1. 「実践: ハンズオンで利用するDashboardをインポートしてみる」で作成した `NGINX Ingress controller` Dashboardを開きます
2. Dashboard上部の `Save` ボタンから、コミットメッセージを入力して保存します
3. forkしたGitHubリポジトリを開き、Step 4で指定した `Target path` 配下にDashboardのJSONがcommitされていることを確認します
4. 試しにGitHub上でJSONの `title` フィールドなどを編集してcommitし、数十秒後にGrafanaの画面を再読み込みすると変更が反映されることを確認します

Dashboardの変更履歴がGit側で追えるようになり、Pull RequestベースでDashboardレビューできる状態になりました。

### Step 6: 後片付け

- Grafana画面の `Provisioning` から接続を `Disconnect` することで同期を停止できます
- 不要になったGitHub Personal Access TokenはGitHub Settingsから削除してください
