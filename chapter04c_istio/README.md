...
- コントロールプレーン: Istiodというシングルバイナリで、トラフィックをproxyするための設定、および管理をします。。このシングルバイナリはPilot, Citadel, Galleyと呼ばれるコンポーネントで構成されています。

  - Pilot: ランタイム時間のproxy設定を行います
  - Citadel: メッシュ内で使用される証明書の発行、更新を行います
  - Galley: メッシュ内設定の検証、取り込み、集約、変換、配布を行います

- データプレーン: サイドカーとして、プロキシ(Envoy)で構成されています。このプロキシは、マイクロサービス間のすべてのネットワーク通信を制御し、メッシュトラフィック全体に関するテレメトリの収集を行います。
